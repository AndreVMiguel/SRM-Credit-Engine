...
public class ExchangeRateService {
    ...
    public ExchangeRateResponse create(ExchangeRateRequest r) {
        ...
        return map(repository.save(new ExchangeRate(base, quote, r.rate(), r.effectiveAt())));
    }
    ...
    public BigDecimal findRate(String base, String quote, Instant at) {
        ...
        var direct = repository.findFirstByBaseCurrencyAndQuoteCurrencyAndEffectiveAtLessThanEqualOrderByEffectiveAtDesc(normalizedBase, normalizedQuote, at);
        if (direct.isPresent()) return direct.get().getRate();
        var inverse = repository.findFirstByBaseCurrencyAndQuoteCurrencyAndEffectiveAtLessThanEqualOrderByEffectiveAtDesc(normalizedQuote, normalizedBase, at).orElseThrow(() -> new BusinessException("Exchange rate not found for " + normalizedBase + "/" + normalizedQuote));
